...
javac是Java的编译器，因为IDE和项目构建软件（maven、gradle等）的强大功能，平常开发的时候很少需要用到javac来编译源码。作为软件开发过程中一个比较基本、底层的知识，往往被人忽略。像我一样，只有在初学Java的时候一知半解地用过几次javac，然后就将他束之高阁的大有人在。但要理解项目构建这样一个很大的话题，理解一下javac也是有必要的。
...
javac A.java -d .
```
如果事情都像这么简单就好了，但我们的工程总会有多个java文件、并且他们对相互依赖、对第三方jar包依赖、分散在各个包中，用`package xxx;`声明。当事情变得复杂的时候，我们应该怎么指挥javac呢？这就是我要写出来的东西。
